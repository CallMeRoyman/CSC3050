    ...
    while (getline(infile, line)) {
        ...
        if (atDotData && !atDotText) {
            ...
            if (dataType.find(".word") != string::npos) {
                ...
                while (ss >> num) {
                    nums.push_back(num);
                    ss >> ch;   /* skip over any trailing whitespace or commas */
                }
                len = nums.size();
                for (int i = 0; i < len; ++i) {
                    ptr = prog + textSize + staticData;
                    *ptr = nums[i] & 0xff;      /* align the binary number to use only beginning 8 bits */
                    *(ptr + 1) = (nums[i] >> 8) & 0xff;
...
void excuteCmd(string machineCode, ifstream& infile, ofstream& outfile, bool& toExit, uint64_t& returnVal) {
    string opCode;
    ...
    if (opCode == "000000") {
        ...
        switch (func) {
            ...
            case 0b100011:
                ...
                break;
            case 0b001100:
                _syscall(infile, outfile, toExit, returnVal);
                break;
            case 0b100110:
                _xor(rd, rs, rt);
                break;
...
int main(int args, char** argv) {
    ...
    /* load .data and .text segement in memory */    
    dataHandler(argv[1]);
    textSeg(argv[2]);
    // for (int i = textSize; i < textSize + staticData; ++i) {
    //     cout << prog[i];
    // }
    // cout << endl;
    ...
    /* start simulation */
    while (currIns >= 0 && currIns < myIns.size()) {

        /* check if the instruction needs to be dump */
        checkpoint_memory(totalIns);
        checkpoint_register(totalIns);

        /* syscall: _exit() || _exit2() */
        if (toExit) break;

        string inst = myIns[currIns];
        reg[_pc] += 4;
        excuteCmd(inst, infile, outfile, toExit, returnVal);
        currIns = (reg[_pc] - startingAddress) >> 2;
        totalIns++;
    }
    checkpoint_memory(totalIns);
    checkpoint_register(totalIns);
    /* locate reg in memory.bin */
    // for (int i = 0; i < 32 + 3; i++) {
    //     reg[i] = i;
    // }
